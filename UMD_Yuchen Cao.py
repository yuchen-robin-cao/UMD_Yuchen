...
df.insert(10, 'Value', abs(df['PRC']) * df['SHROUT'], True) # create value
df.insert(11, 'original value', df['Value'])
df['Value'] = df['Value'].shift(1) # shift value by 1
coerce = pd.to_numeric(df['RET'], errors = 'coerce') # coerce RET to numeric (non-convertible values would be recorded as NaN)
df.insert(12, 'Mean_RET', (coerce.rolling(13).sum() - coerce.rolling(2).sum()) / 11) # mean_return from t-12 to t-2
df.insert(13, 'Is_Valid', pd.Series(df['PERMNO'].rolling(13).mean() == df['PERMNO'])) # check whether Mean_RET is computed for the same stock
...
dates = np.unique(df['date']) # derive candidate dates

UMD = []
UMD_w = []

for i in range(12, len(dates)): # we start from the 12th in order to reserve moving ave period
    ...
    #ret_C_indx = np.where(df['RET'] == 'C')[0][:] # since we have 'coverced_RET', this is no longer needed
    ...
    SH_ret = np.mean(sample_value_dropna['Mean_RET'].loc[SH_indx])
    BH_ret = np.mean(sample_value_dropna['Mean_RET'].loc[BH_indx])
    SL_ret = np.mean(sample_value_dropna['Mean_RET'].loc[SL_indx])
    BL_ret = np.mean(sample_value_dropna['Mean_RET'].loc[BL_indx])
    
    ret = 0.5 * (SH_ret + BH_ret) - 0.5 * (SL_ret + BL_ret) # equal weights
    ...
    SH_ret_w = np.nansum(np.array(SH_value.loc[SH_indx]) * np.array(df['Coerced_RET'].loc[SH_indx]))
    BH_ret_w = np.nansum(np.array(BH_value.loc[BH_indx]) * np.array(df['Coerced_RET'].loc[BH_indx]))
    SL_ret_w = np.nansum(np.array(SL_value.loc[SL_indx]) * np.array(df['Coerced_RET'].loc[SL_indx]))
    BL_ret_w = np.nansum(np.array(BL_value.loc[BL_indx]) * np.array(df['Coerced_RET'].loc[BL_indx]))
    
    ret_w = (0.5 * (SH_ret_w + BH_ret_w) - 0.5 * (SL_ret_w + BL_ret_w)) # value weights
    UMD_w.append(ret_w)
    UMD.append(ret)
    #print(ret)
    #print(ret_w)
...
#UMD = pd.Series(UMD)
#UMD_w = pd.Series(UMD_w)
#UMD.to_excel('UMD.xls')
#UMD_w.to_excel('UMD_w.xls')
